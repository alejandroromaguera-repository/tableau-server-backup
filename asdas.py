SELECT 
    Z.*,
    SUM(CASE WHEN Z.RN_LINEA = 1 THEN Z.LINETOTAL_PED ELSE 0 END)
    OVER (PARTITION BY Z.FECHADEF) AS TOTAL_PEDIDOS_FECHA
FROM
(
SELECT 
    W.*,
    ROW_NUMBER() OVER (PARTITION BY W.DOCENTRY_PED, W.LINENUM_PED
                        ORDER BY W.DOCENTRY_FACT NULLS LAST) AS RN_LINEA
FROM
(
SELECT 
    V.*,
...
LEFT JOIN STD_MEX_BI_ENTREGAS entr ON entr.BASETYPE = '17' AND entr.BASEENTRY = ped.DOCENTRY_PED AND entr.BASELINE = ped.LINENUM_PED
...
WHERE W.GROUPED_CLIENT_NAME_CLI LIKE 'LIVERPOOL' AND EXTRACT(MONTH FROM W.FECHADEF) = 8 AND EXTRACT(YEAR FROM W.FECHADEF) = 2026 AND W.DOCNUM_PED = '21750';
)Z
WHERE Z.GROUPED_CLIENT_NAME_CLI LIKE 'LIVERPOOL' AND EXTRACT(MONTH FROM Z.FECHADEF) = 8 AND EXTRACT(YEAR FROM Z.FECHADEF) = 2026 AND Z.DOCNUM_PED = '21750' AND Z.RN_LINEA = 1
;
